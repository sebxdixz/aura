...
def create_jira_ticket(
    ...
    with start_span("ticketing.jira.create", **{"incident.id": incident_id, "tenant.id": tenant_id}):
        ...
        try:
            resp = _http_post(f"{base_url}/rest/api/3/issue", payload)
...
def fetch_jira_ticket_status(ticket_id: str) -> str:
    with start_span("ticketing.jira.fetch_status", **{"ticket.id": ticket_id, "ticket.provider": "jira"}):
        if not _jira_available():
            return "Created"

        base_url = os.getenv("JIRA_BASE_URL", "").rstrip("/")
        payload = _http_get(f"{base_url}/rest/api/3/issue/{ticket_id}?fields=status")
